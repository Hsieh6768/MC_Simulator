...
std::unique_ptr<Skill> SkillManager::createSkill(const string& skillType, int level) {
    if (skillType == "fireball") {
        auto skill = std::make_unique<AttackSkill>("火球术", "发射一个火球攻击敌人", 15, 2, 20, 1.2);
        for (int i = 1; i < level; i++) skill->upgrade();
        return skill;
    }
    else if (skillType == "heal") {
        auto skill = std::make_unique<HealSkill>("治疗术", "恢复自身生命值", 20, 3, 30);
        for (int i = 1; i < level; i++) skill->upgrade();
        return skill;
    }
    else if (skillType == "powerup") {
        auto skill = std::make_unique<BuffSkill>("力量强化", "提升攻击力和防御力", 25, 4, 3, 5, 3);
        for (int i = 1; i < level; i++) skill->upgrade();
        return skill;
    }
    return nullptr;
